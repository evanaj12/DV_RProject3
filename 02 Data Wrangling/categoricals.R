...
myplot <- function(df, x) {
  names(df) <- c("x", "n")
  ggplot(df, aes(x=x, y=n)) + geom_point()+ scale_x_discrete(x)+ theme(axis.text.x = element_text(size=6, angle=90, vjust=0.5))+labs(title=paste("Categorical Plot - ", x)) + theme(plot.title = element_text(size=40, face="bold", vjust=1, family="Times"))
}
...
if (length(ClimateList) > 0) {
  ...
  print(ClimateList[[1]], vp = viewport(layout.pos.row = 1, layout.pos.col = 1:3))
  print(ClimateList[[2]], vp = viewport(layout.pos.row = 1, layout.pos.col = 4:6))
  print(ClimateList[[3]], vp = viewport(layout.pos.row = 2, layout.pos.col = 1:6, height = 0.5))
  print(ClimateList[[4]], vp = viewport(layout.pos.row = 3, layout.pos.col = 1:6, height = 0.5))
  dev.off()
}
